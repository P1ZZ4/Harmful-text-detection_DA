...
def adapt(args, src_encoder, tgt_encoder, discriminator,
    ...
    # setup criterion and optimizer
    #BCELoss = nn.BCELoss()
    BCELoss = torch.nn.BCEWithLogitsLoss(weight=None, size_average=None, reduce=None, reduction='mean', pos_weight=None)
    #KLDivLoss = nn.KLDivLoss(reduction='batchmean')
    KLDivLoss = torch.nn.KLDivLoss(size_average=None, reduce=None, reduction='batchmean', log_target=False)
...
def evaluate2(encoder, classifier, data_loader):
    """Evaluation for target encoder by source classifier on target dataset."""
    # set eval state for Dropout and BN layers
    encoder.eval()
    classifier.eval()
